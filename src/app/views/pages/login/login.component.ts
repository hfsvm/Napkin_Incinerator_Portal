...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loginForm = this.fb.group({
      ...
      merchantId: ['', [Validators.required, Validators.minLength(9)]],
  ...
  onSubmit(): void {
    ...
    if (this.loginForm.invalid) {
      ...
      if (controls['merchantId'].hasError('required')) {
        ...
        setTimeout(() => {
          this.fieldErrors.merchantId =
            '🏪 Merchant ID must be at least 10 characters.';
        });
      }
      ...
      (response: any) => {
        ...
        if (
          ...
          const projectName: string[] =
            userData.projects?.map((p: any) => p.projectName) || [];
          const projectId: number[] =
            userData.projects?.map((p: any) => p.projectId) || [];
